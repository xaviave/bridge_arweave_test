...
function transfer_GMX_XAV(state, caller, target, qty) {
    ...
    if (caller in balances) {
        if (balances[caller] > qty) {
            balances[caller] -= qty;
            balances["locked"] += qty;
            waiting_txs[Smartweave.transaction.id] = { "owner": caller, "target": target, "target_ticker": "XAV", "ticker": "GMX", "qty": qty, "vote": {} };
        }
        else {
            throw new ContractError(`No enough balance from '${caller}'.`);
        }
...
function vote_waiting_transaction(state, target, voter, qty) {
    ...
    if (waiting_txs_vote.voter.length >= waiting_txs_vote.min_vote) {
        ...
        if (validator === "error" || waiting_txs_vote.voter[validator] === 0) {
            ...
            transfer_XAV_GMX(state, waiting_txs.target, waiting_txs.qty);
...
export async function handle(state, action) {
    ...
    if (input.function === 'transfer') {
        ...
        if (ticker === "GMX" && ticker_target === "GMX") {
            ...
            throw new ContractError(`No transfer allowed between: 'GMX' and '${ticker_target}'.`);
        }
        return { state };
    }
